    ...
    <div className="category-side-container">
      ...
      <div className="category-side">
        {categoryes.map((categorye) => (
         ...
         >
            <p  onClick={() => handleCategoryClick(categorye.value)}>{categorye.label}</p>
                ...
                if (subcategorye.parentCategoryId === categorye.value) {
                  return (
                    <ul key={subcategorye.id}>
                      <li className="drop-down-titel"  onClick={() => handleCategoryClick(subcategorye.id)}>{subcategorye.label} </li>
                    <div>
                    {subSubCategoryes.map((subcategorye2,index) => {
                        if (
                          subcategorye2.parentCategoryId === subcategorye.id
                        ) {
                          return <li key={index}onClick={() => handleCategoryClick(subcategorye2.id)} >{subcategorye2.label}</li>;
                        }
                      })}
                    </div>
                    </ul>
